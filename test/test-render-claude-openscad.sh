...
cleanup() {
    rm -f test-render-claude-openscad.md
    rm -rf output/
}
...
if ../render.sh test-render-claude-openscad.lmd > test-render-claude-openscad.md 2>/dev/null; then
...
if [ -f "test-render-claude-openscad.md" ] && [ -s "test-render-claude-openscad.md" ]; then
    pass "Output file created ($(wc -c < test-render-claude-openscad.md) bytes)"
    echo "   Preview (first 10 lines):"
    head -10 test-render-claude-openscad.md | sed 's/^/     /'
else
    fail "Output file not created or empty"
fi

info "Test 3: Verify OpenSCAD code is in the markdown"
if grep -qE '```(openscad|scad)' test-render-claude-openscad.md; then
    pass "OpenSCAD code block found in markdown"
    # Extract and validate the actual OpenSCAD code (check entire block)
    if sed -n '/```\(openscad\|scad\)/,/```/p' test-render-claude-openscad.md | grep -qE '(module|cube|cylinder|sphere|translate|rotate|difference|union)'; then
        pass "Valid OpenSCAD syntax inside markdown"
        echo "   Code preview:"
        grep -A 5 -E '```(openscad|scad)' test-render-claude-openscad.md | head -6 | sed 's/^/     /'
    else
