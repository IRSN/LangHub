...
cleanup() {
    rm -f test-render-copilot-openscad.md
    rm -rf output/
}
...
if ../render.sh test-render-copilot-openscad.lmd > test-render-copilot-openscad.md 2>/dev/null; then
...
if [ -f "test-render-copilot-openscad.md" ] && [ -s "test-render-copilot-openscad.md" ]; then
    pass "Output file created ($(wc -c < test-render-copilot-openscad.md) bytes)"
    echo "   Preview (first 10 lines):"
    head -10 test-render-copilot-openscad.md | sed 's/^/     /'
else
    fail "Output file not created or empty"
fi

info "Test 3: Verify OpenSCAD code is in the markdown"
if grep -qE '```(openscad|scad)' test-render-copilot-openscad.md; then
    pass "OpenSCAD code block found in markdown"
    # Extract and validate the actual OpenSCAD code (check more lines)
    if sed -n '/```\(openscad\|scad\)/,/```/p' test-render-copilot-openscad.md | grep -qE '(module|cube|cylinder|sphere|translate|rotate|difference|union)'; then
        pass "Valid OpenSCAD syntax inside markdown"
        echo "   Code preview:"
        grep -A 5 -E '```(openscad|scad)' test-render-copilot-openscad.md | head -6 | sed 's/^/     /'
    else
